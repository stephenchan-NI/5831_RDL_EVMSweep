...
namespace NationalInstruments.ReferenceDesignLibraries.SA
{
    ...
    public static class RFmxNR
    {
        ...
        public static void ConfigureModacc(RFmxNRMX nr, ModAccConfiguration modAccConfig, string selectorString = "")
        {
            ...
            nr.ModAcc.Configuration.SetDCSubcarrierRemovalEnabled(selectorString, RFmxNRMXModAccDCSubcarrierRemovalEnabled.True);
            nr.ModAcc.Configuration.SetSynchronizationMode(selectorString, RFmxNRMXModAccSynchronizationMode.Frame);
            nr.Commit(selectorString);
        }
        ...
        public static void refLevelOptimization(RFmxNRMX nr, string selectorString = "", string resultName = "")
        {
            List<double> evm_percent = new List<double>();
            List<double> ref_level_list = new List<double>();
            double refLevel;
            nr.AutoLevel(selectorString, 0.01, out refLevel);
            for (int i = 0; i < 21; i++)
            {
                ref_level_list.Add(refLevel + 1 - (0.5 * i));
            }
            foreach (double ref_level in ref_level_list)
            {
                double evm;
                nr.SetReferenceLevel(selectorString, ref_level);
                nr.Initiate(selectorString, resultName);
                if (nr.WaitForMeasurementComplete(selectorString, -1) > 0) //Warnings are positive numbers
                {
                    break;
                }
                nr.ModAcc.Results.GetCompositeRmsEvmMean(selectorString, out evm);
                evm_percent.Add(evm);
            }
            double min = evm_percent.Min();
            int min_index = evm_percent.IndexOf(min);
            nr.SetReferenceLevel(selectorString, ref_level_list[min_index]);
            nr.ModAcc.Configuration.SetAveragingEnabled(selectorString, RFmxNRMXModAccAveragingEnabled.True);
            nr.ModAcc.Configuration.SetAveragingCount(selectorString, 10);
